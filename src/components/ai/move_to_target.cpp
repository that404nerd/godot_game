...
void BTMoveToTarget::_bind_methods()
{
  ...
  GD_BIND_PROPERTY(BTMoveToTarget, should_move_to_player, Variant::BOOL);
}

BT::Status BTMoveToTarget::_tick(double delta)
{
  if(move_type == MoveType::RUN)
    return m_AIManagerInst->_chase(delta, should_rotate_towards_player, should_move_to_player);
  else
    return m_AIManagerInst->_patrol(delta, should_rotate_towards_player, should_move_to_player);
